...
class BenchmarkPlugin extends \Yaf\Plugin_Abstract
{
	...
	public function dispatchLoopShutdown(\Yaf\Request_Abstract $request, \Yaf\Response_Abstract $response) 
	{
		...
		if ($time > $configs->application->benchmark->time)
		{
			file_put_contents(ROOT_PATH . $configs->application->benchmark->log_file, 'benchmark:' . $request->getRequestUri() . ':' . $time . "s" . ':' . (memory_get_usage(true) / 1024) . 'kb' . "\n\r", FILE_APPEND);
		}
		unset($configs);
	}
}