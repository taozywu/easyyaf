...
class Loader {
	...
	public function autoload($class)
	{
		...
		$appDir = G::get('directory');
		$directory = '';
		if ($this->isCategoryType($class, self::YAF_LOADER_MODEL))
		{
			//this is a model
			$directory = $appDir . DIRECTORY_SEPARATOR . self::YAF_MODEL_DIRECTORY_NAME . DIRECTORY_SEPARATOR;
			$class = $this->resolveCategory($class, self::YAF_LOADER_MODEL);
		}
		else if ($this->isCategoryType($class, self::YAF_LOADER_PLUGIN))
		{
			//this is a plugin
			$directory = $appDir . DIRECTORY_SEPARATOR . self::YAF_PLUGIN_DIRECTORY_NAME;
			$class = $this->resolveCategory($class, self::YAF_LOADER_PLUGIN);
		}
		else if ($this->isCategoryType($class, self::YAF_LOADER_CONTROLLER)
		)
		{
			//this is a controller
			$directory = $appDir . DIRECTORY_SEPARATOR . self::YAF_CONTROLLER_DIRECTORY_NAME;
			$class = $this->resolveCategory($class, self::YAF_LOADER_CONTROLLER);
		}
		else if ($this->isCategoryType($class, self::YAF_LOADER_DAO) || $this->isCategoryType($class, self::YAF_LOADER_SERVICE)
		)
		{
			//this is used internally
			$directory = $appDir . DIRECTORY_SEPARATOR . self::YAF_MODEL_DIRECTORY_NAME;
		}
		if ($directory != '' && $appDir == '')
		{
			Exception::trigger_error('Couldn\'t load a framework MVC class without an ' . 'Yaf_Application initializing ', E_USER_WARNING);
		}
		$splAutoLoadIni = G::iniGet('yaf.use_spl_autoload');
		...
		else
		{
			if ($splAutoLoadIni == false)
			{
				Exception::trigger_error('Could not find script ' . ($directory != '' ? $directory : $this->resolveDirectory($class)) . DIRECTORY_SEPARATOR . $this->resolveClass($class), E_USER_WARNING);
			}
			else
			{
				return false;
